...
import Chatbot.torch.optim as optim
import random
...
def maskNLLLoss(inp, target, mask):
    nTotal = mask.sum()
    _shape = inp.shape
    inp = inp.view(_shape[1], _shape[2])
...
if __name__ == '__main__':
    ...
    decoder = LuongAttnDecoderRNN(embedding, hidden_size, voc.num_words, model_name, decoder_n_layers, dropout)
    if isLoadModel and modelFileIsExist:
